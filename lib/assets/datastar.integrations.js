import {
    ...
    startPeeking,
    stopPeeking,
} from '/__webui-datastar/integrations/datastar.min.js';

let isWebUIReady = false;
let resolveWebUIReady;

const dispatchLifecycle = (type, el, argsRaw = {}) => {
...
const webuiReady = new Promise((resolve) => {
    resolveWebUIReady = resolve;
});
    ...
    apply: async (context, ipcFunctionName, options = {}) => {
        ...
        if (contentType === 'json' && data === undefined) {
            ...
            if (el === formElement && evt instanceof SubmitEvent) {
                ...
                cleanupFn = () =>
                    formElement.removeEventListener('submit', preventDefault);
            }
